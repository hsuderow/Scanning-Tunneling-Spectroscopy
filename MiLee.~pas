...
function TValProm.absvec2(a,b:integer):extended;
begin
  result:=sqrt(Power(a,2)+Power(b,2))
end;

function TValProm.scalarproduct2(ax,ay,bx,by:integer):integer;
begin
  result:=ax*bx+ay*by;
end;

function TValProm.angbetvec2(ax,ay,bx,by:integer):extended;
...
procedure TValProm.Triangulation(l,k:integer; m_cola3:array of TPart; var m_con:array of TConnection);
var
//   Ext : string;
    ...
    then
      ...
      begin
        ...
        if (ys>min1) and (ys<max1) then
          if (abs(ys-min1)>1E-9) and (abs(max1-ys)>1E-9) then
          begin
            if ax<bx then
            begin
              min1:=ax;
            ...
            if (cx<max1) and (cx>min1) then
              if (abs(cx-min1)>1E-9) and (abs(max1-cx)>1E-9)
              then a:=true else a:=false;
          end;
      end
      else
...
// Setting connections inside the parts:
    ...
    begin
      ...
      begin
        for j:=0 to length(m1[i])-1 do
        begin
          s1:=FloatToStr(m1[i,j]);
          if length(s1)<8 then
            repeat
              s1:=s1+'0'
            until length(s1)=8;
...
//Triangulation:
    ...
    for i:=0 to length(m_point)-1 do
    begin
      SetLength(alpha, length(alpha)+1);
      sum2:=2*Pi;
      ...
      begin
        ...
        begin
          inc(o);
          SetLength(alpha[i],length(alpha[i])+1);
          for k:=0 to length(m_con)-1 do
            if k<>j then
              if ((m_con[k].P1.x=m_Point[i].x) and (m_con[k].P1.y=m_point[i].y)) or
                 ((m_con[k].P2.x=m_Point[i].x) and (m_con[k].P2.y=m_point[i].y))
              then
              begin
                if (m_con[j].P1.x=m_Point[i].x) and (m_con[j].P1.y=m_point[i].y)
                then
                  if (m_con[k].P1.x=m_Point[i].x) and (m_con[k].P1.y=m_point[i].y)
                  then
                    sum:=angbetvec2(m_con[j].P2.x-m_con[j].P1.x,m_con[j].P2.y-m_con[j].P1.y,
                                    m_con[k].P2.x-m_con[k].P1.x,m_con[k].P2.y-m_con[k].P1.y)
                  else
                    sum:=angbetvec2(m_con[j].P2.x-m_con[j].P1.x,m_con[j].P2.y-m_con[j].P1.y,
                                    m_con[k].P1.x-m_con[k].P2.x,m_con[k].P1.y-m_con[k].P2.y)
                else
                  if (m_con[k].P1.x=m_Point[i].x) and (m_con[k].P1.y=m_point[i].y)
                  then
                    sum:=angbetvec2(m_con[j].P1.x-m_con[j].P2.x,m_con[j].P1.y-m_con[j].P2.y,
                                    m_con[k].P2.x-m_con[k].P1.x,m_con[k].P2.y-m_con[k].P1.y)
                  else
                    sum:=angbetvec2(m_con[j].P1.x-m_con[j].P2.x,m_con[j].P1.y-m_con[j].P2.y,
                                    m_con[k].P1.x-m_con[k].P2.x,m_con[k].P1.y-m_con[k].P2.y);
                if sum<sum2 then sum2:=sum;
              end;
          if sum2<>2*Pi then alpha[i,o-1]:=sum2 else dec(o);
          sum2:=2*Pi;
        end;
      end;
      o:=0;
    ...
    then
    begin
      text_writeln(g, 'x'#9'y'#9'numero de los vecinos'#9'angulos'#9+#9+#9'vecinos');
      for i:=0 to length(m_point)-1 do
      begin
        o:=0;
        for j:=0 to length(m_con)-1 do
        ...
        s2:=IntToStr(o);
        s3:='';
        for k:=0 to length(alpha[i])-1 do
          s3:=s3+#9+FloatToStr(alpha[i,k]);
        text_write(g, s+#9+s1+#9+#9+s2+#9+#9);
        text_writeln(g,'');
        k:=0;
        if length(m_con)>1 then
          for j:=0 to length(m_con)-1 do
            if ((m_con[j].P1.x=m_Point[i].x) and (m_con[j].P1.y=m_point[i].y))
            then
            begin
              text_write(g,#9+#9+#9+#9+#9+FloatToStr(alpha[i,k])+#9);
              if length(FloatToStr(alpha[i,k]))<16 then text_write(g,#9);
              text_writeln(g,'('+IntToStr(m_con[j].P2.x)+','+IntToStr(m_con[j].P2.y)+')');
              inc(k);
            end
            else
              ...
              begin
                text_write(g,#9+#9+#9+#9+#9+FloatToStr(alpha[i,k])+#9);
                if length(FloatToStr(alpha[i,k]))<16 then text_write(g,#9);
                text_writeln(g,'('+IntToStr(m_con[j].P1.x)+','+IntToStr(m_con[j].P1.y)+')');
                inc(k);
              end;
        text_writeln(g,'');
      end;
