...
type
  TAuxGraph1 = class(TForm)
    ...
    procedure GenGraphMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
...
coll,i: Integer;
begin
if (ActiveCurveNr.Value>100) then coll:=Round(ActiveCurveNr.Value/10)
else coll:=Round(ActiveCurveNr.Value);

ActiveCurveNr.Color:=GenGraph[coll].Color;
...
for i:=1 to MaxGraph do
 begin
 if i=ActiveCurveNr.Value then GenGraph[i].PlotPoints:=False
 else GenGraph[i].PlotPoints:=True;
 end;
GenGraph.Update;
...
for j:=1 to SpinEdit1.Value do
 begin
  ...
  Form2.RichEdit1.Clear;
    for i:=0 to 127 do
     begin
      Form2.RichEdit1.Text:=Form2.RichEdit1.Text+DS.comment[i];
     end;
  AuxGraph1.Show;
...
begin
  ...
  *}
  offsetval:=StrtoFloat(ValProm.Edit11.Text);
//  DiffIyV:=StrtoFloat(ValProm.Edit2.Text);
  ...
  CurvaADerivar.n:=Datos2.n;
   pderi:=StrtoInt(ValProm.PDeriva.Text);
  for i:=1 to DS.nrows-1 do
  begin
  ...
  if ValProm.Baseline.Checked then
  begin
  if cont>0 then for i:=1 to Datos2.n do
      ...
      if (CheckBox5.checked) then
          begin
           pderi:=StrtoInt(ValProm.Edit8.Text);
          for i:=0 to  Datos2.n-1 do
                begin
  ...
  begin
  for i:=0 to Datos2.n-1 do
   GenGraph[ActiveCurveNr.Value].AddPoint(Datos2.x[i],Datos2.y[i]);
  end;
  Form2.Label4.Caption:=InttoStr(Datos2.n);   // Write point number
  Interv:=2*abs(GenGraph[ActiveCurveNr.Value].x[0])/Datos2.n;
  Form2.Label14.Caption:=FloattoStrF(Interv,ffGeneral,5,4);
  Integ:=0;
  for i:=0 to Datos2.n-1 do
   Integ:=Integ+GenGraph[ActiveCurveNr.Value].y[i];                                
  Integ:=Integ*Interv;
  Form2.Label6.Caption:=FloattoStrF(Integ,ffGeneral,5,4);
  ZBC:=GenGraph[ActiveCurveNr.Value].y[Round(Datos2.n/2-offsetval/Interv)];
  Form2.Label8.Caption:=FloattoStrF(ZBC,ffGeneral,5,4);
end;
...
var
i,j,Size:Integer;
MiX,MiY: Array[0..10000] of Extended;
...
begin
  ...
  for i:=0 to Size-1 do
   begin
   MiX[i]:=GenGraph[j].x[i]+MiX[i];
   MiY[i]:=GenGraph[j].y[i]+MiY[i];
   end;
  GenGraph[j].Clear;
end;
  for i:=0 to 10000 do
   begin
   MiX[i]:=MiX[i]/ActiveCurveNr.Value;
   MiY[i]:=MiY[i]/ActiveCurveNr.Value;
   end;
DeleteAllClick(nil);
for i:=0 to Size-1 do
  begin
  GenGraph[ActiveCurveNr.Value].AddPoint(MiX[i],MiY[i]);
  end;
GenGraph.Update;
end;
...
var
Fich,j,i,cont: Integer;
PromBeginval,PromEndval,offsetval,DiffIyV: Double;
Prom: Double;
DatosX,DatosY: Array [0..250] of Extended;
...
while (Fich<b.Count) do
begin
  Application.ProcessMessages;
  b_offset:=b.GetEntryOffset(Fich) ;
  LoadDataSetFromBlock(blqname,b_offset,DS) ;
  Label6.Caption:=InttoStr(Fich);
  ...
  for i:=1 to Datos2.n-1 do
   begin
   DatosX[i]:=DatosX[i]+ CurvaDerivada.x[i];
   DatosY[i]:=DatosY[i]+ CurvaDerivada.y[i];
  end;
  end     // if checkderiva
  else
  begin
  for i:=0 to Datos2.n-1 do
   begin
    DatosX[i]:=DatosX[i]+ Datos2.x[i];
    DatosY[i]:=DatosY[i]+ Datos2.y[i];
   end;
  end;
Fich:=Fich+1;
end;
  ...
  for i:=0 to Datos2.n-1 do
   begin
    DatosX[i]:=DatosX[i]/Fich;
    DatosY[i]:=DatosY[i]/Fich;
   end;

 for i:=0 to Datos2.n-1 do
  begin
  GenGraph[ActiveCurveNr.Value].AddPoint(DatosX[i],DatosY[i]);
  end;
GenGraph.Update;
...
procedure TAuxGraph1.GenGraphMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
Label8.Caption:=FloattoStrF(GenGraph.GetX(X),fffixed,5,4);
Label12.Caption:=FloattoStrF(GenGraph.GetY(Y),fffixed,5,4);

end;

procedure TAuxGraph1.Button13Click(Sender: TObject);
var
PlotNr: Integer;
begin
PlotNr:=StrtoInt(GotoPlotNr.Text);
PlotNr:=plotNr+2;
GotoPlotNr.Text:=InttoStr(PlotNr);
Button4Click(nil);
end;

procedure TAuxGraph1.Button14Click(Sender: TObject);
var
PlotNr: Integer;
begin
PlotNr:=StrtoInt(GotoPlotNr.Text);
PlotNr:=plotNr-2;
if (PlotNr<0) then PlotNr:=0;
GotoPlotNr.Text:=InttoStr(PlotNr);
Button4Click(nil);
end;
