...
uses Graphiccs, BCSFit, HeaderSPM, Math;
...
//      c:TCircle;
  begin
    ...
    u:=(by-cy)*Power(ax+ay,2)+(cy-ay)*Power(bx+by,2)+(ay-by)*Power(cx+cy,2);
    v:=(cx-bx)*Power(ax+ay,2)+(ax-cx)*Power(bx+by,2)+(bx-ax)*Power(cx+cy,2);
    if d<>0 then
    begin
      middlex:=0.5*u/d;
      middley:=0.5*v/d;
...
//    middlex:=((Power(ay,2)+Power(ax,2)-Power(by,2)-Power(bx,2))/2.0+middley*(by-ay))/(ax-bx);
      ...
      if (Power(tx-middlex,2)+Power(ty-middley,2))>Power(radius,2) then b:=1;
  ...
  begin
    ...
    then
      ...
      then a:=false
        ...
        if ax=cx then a:=true else a:=false
    else
    begin
      if bx<>ax then
      begin
        ...
        if m1=m2
        then
          if cy=ay+m1*(cx-ax) then a:=true else a:=false
        else
        begin
          xs:=(cy-ay+m1*ax-m2*cx)/(m1-m2);
          if (xs>min1) and (xs<max1) and (xs>min2) and (xs<max2) then a:=true else a:=false;
        end;
      end;
    end;
    Cut:=a;
  ...
  begin
    ...
    for n:=0 to length(m_cola[0].num)-1 do  // for all points in the left part
      ...
      begin
          ...
          for j:=i+1 to length(m_cola[0].num)-1 do //  of pairs in the left part
            ...
            begin
              if Cut(m_cola[0].num[m_sort_left[n].i].x,m_cola[0].num[m_sort_left[n].i].y,
                     m_cola[1].num[m_sort_right[0].i].x,m_cola[1].num[m_sort_right[0].i].y,
        ...
        begin
          ...
          m_con[k].P2:=m_cola[1].num[m_sort_right[0].i];
          a:=false;
        end
        else a:=true;
      end;
  ...
  begin
      ...
      begin
        ...
        if (alpha<beta) and (m_cola[1].num[i].x<>m_con[k].P2.x) and (m_cola[1].num[i].y<>m_con[k].P2.y) then
      ...
      begin
        ...
        if (alpha<beta) and (m_cola[0].num[i].x<>m_con[k].P1.x) and (m_cola[0].num[i].y<>m_con[k].P1.y) then
        // only this angles are orientated to the left part and it is not the point on the baseline
        begin
          a:=false;
      ...
      begin
        ...
        begin
          ...
          for i:=0 to o+1 do // for all values
            if not(b1) then
            begin
              for j:=i+1 to o do // for all other values
                if not((m_cola[1].num[i+1].x=m_con[k].P1.x) and
                       (m_cola[1].num[i+1].y=m_con[k].P1.y)) and
                   not((m_cola[1].num[i+1].x=m_con[k].P2.x) and
                       (m_cola[1].num[i+1].y=m_con[k].P2.y)) and
                   not((m_cola[1].num[j+1].x=m_con[k].P1.x) and
                       (m_cola[1].num[j+1].y=m_con[k].P1.y)) and
                   not((m_cola[1].num[j+1].x=m_con[k].P2.x) and
                       (m_cola[1].num[j+1].y=m_con[k].P2.y))  // not the same like the baseline
                then
                begin
                  if CheckDistance(m_con[k].P1.x,m_con[k].P1.y,
                                   m_con[k].P2.x,m_con[k].P2.y,
                                   m_cola[1].num[i+1].x,m_cola[1].num[i+1].y,
                                   m_cola[1].num[j+1].x,m_cola[1].num[j+1].y)=0
                  then b1:=true;   // point is inside the circle
                end
                else
                  if ((m_cola[0].num[i+1].x=m_con[k].P1.x) and
                      (m_cola[0].num[i+1].y=m_con[k].P1.y)) or
                     ((m_cola[0].num[i+1].x=m_con[k].P2.x) and
                      (m_cola[0].num[i+1].y=m_con[k].P2.y)) // point is not the baseline
                  then b1:=true;
...
//                if (m_sort_left[j].alpha=0.0) and (b2) then p:=p-1 else b2:=false;
              ...
              for n:=i+1 to p do
                if not((m_cola[1].num[i+1].x=m_con[k].P1.x) and
                   ...
                   not((m_cola[1].num[i+1].x=m_con[k].P2.x) and
                       (m_cola[1].num[i+1].y=m_con[k].P2.y)) and
                   not((m_cola[0].num[p+1].x=m_con[k].P1.x) and
                       (m_cola[0].num[p+1].y=m_con[k].P1.y)) and
                   not((m_cola[0].num[p+1].x=m_con[k].P2.x) and
                       (m_cola[0].num[p+1].y=m_con[k].P2.y))
                then
                begin
                  if CheckDistance(m_con[k].P1.x,m_con[k].P1.y,
                                   m_con[k].P2.x,m_con[k].P2.y,
                                   m_cola[1].num[i+1].x,m_cola[1].num[i+1].y,
                                   m_cola[0].num[p+1].x,m_cola[0].num[p+1].y)=0
                  then b1:=true;
                end
                else b1:=true;
              if not(b1) then // Delaunay condition works --> the candidate of the right side is found
              begin
                Pr.x:=m_cola[1].num[i+1].x;
                Pr.y:=m_cola[1].num[i+1].y;
                b1:=true;
              end
              else b1:=false;
            end;
        ...
        begin
          ...
          begin
            ...
            begin
              for j:=i+1 to o do
                if not((m_cola[0].num[i+1].x=m_con[k].P1.x) and
                       (m_cola[0].num[i+1].y=m_con[k].P1.y)) and
                   not((m_cola[0].num[i+1].x=m_con[k].P2.x) and
                       (m_cola[0].num[i+1].y=m_con[k].P2.y)) and
                   not((m_cola[0].num[j+1].x=m_con[k].P1.x) and
                       (m_cola[0].num[j+1].y=m_con[k].P1.y)) and
                   not((m_cola[0].num[j+1].x=m_con[k].P2.x) and
                       (m_cola[0].num[j+1].y=m_con[k].P2.y))

                then
                begin
                  if CheckDistance(m_con[k].P1.x,m_con[k].P1.y,
                                   m_con[k].P2.x,m_con[k].P2.y,
                                   m_cola[0].num[i+1].x,m_cola[0].num[i+1].y,
                                   m_cola[0].num[j+1].x,m_cola[0].num[j+1].y)=0
                  ...
                  if ((m_cola[0].num[i+1].x=m_con[k].P1.x) and
                      (m_cola[0].num[i+1].y=m_con[k].P1.y)) or
                     ((m_cola[0].num[i+1].x=m_con[k].P2.x) and
                      (m_cola[0].num[i+1].y=m_con[k].P2.y))
                  then b1:=true;
              b2:=true;
  ...
  //              if (m_sort_right[j].alpha=0.0) and (b2) then p:=p-1 else b2:=false;
              ...
              for n:=i+1 to p do
                if not((m_cola[0].num[i+1].x=m_con[k].P1.x) and
                   ...
                   not((m_cola[0].num[i+1].x=m_con[k].P2.x) and
                       (m_cola[0].num[i+1].y=m_con[k].P2.y)) and
                   not((m_cola[1].num[p+1].x=m_con[k].P1.x) and
                       (m_cola[1].num[p+1].y=m_con[k].P1.y)) and
                   not((m_cola[1].num[p+1].x=m_con[k].P2.x) and
                       (m_cola[1].num[p+1].y=m_con[k].P2.y))
                then
                begin
                  if CheckDistance(m_con[k].P1.x,m_con[k].P1.y,
                                   m_con[k].P2.x,m_con[k].P2.y,
                                   m_cola[0].num[i+1].x,m_cola[0].num[i+1].y,
                                   m_cola[1].num[p+1].x,m_cola[1].num[p+1].y)=0
                  then b1:=true  // b when point inside the cirle
                end
                else b1:=true;
              if not(b1) then   // when all other points outside the circle
              begin
                Pl.x:=m_cola[0].num[i+1].x;
                Pl.y:=m_cola[0].num[i+1].y;
                b1:=true;
              end
              else b1:=false;
            end;
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          else // triangulation for two parts is finished
            c:=true;
        end;
...
var
   Ext : string;
   i,j,l,k,n,o,p,amp,PosFin,PosFinFin,SizeHead: Longint;
   WsxmComment: Array [0..5000] of Char;
   AmpZ: Array [0..10] of Char;
   AmplZ: Single;
   Encuentra: Boolean;
...
//   m:array of array of string;
   ...
   ex,ey,ez: extended;
   ok: boolean;
(*  function (m:array of TPoint):array of TPoint;
  var i,j,k,check,x,y:integer;
      a:boolean;
      m1,m2:array of TPoint;
      P:array[0..1] of integer;
   function PointMore(m:array of TPoint, i:integer):array[0..1] of integer;
   var j:integer;
   begin
     for j:=0 to length(m2)-1 do
       if m2[j]=m[i] then a:=true;
     if not(a) then
     begin
       for j:=0 to length(m)-1 do
       begin
         if ((m[j].x=m[i].x-1) or (m[j].x=m[i].x+1)) and (m[j].y=m[i].y) then P:=PointMore(m,j);
         if ((m[j].y=m[i].y-1) or (m[j].y=m[i].y+1)) and (m[j].x=m[i].x) then P:=PointMore(m,j);;
       end;
       x:=x+P[0];
       y:=y+P[1];
     end;
  ...
  begin
    ...
    begin
      ...
      for j:=0 to length(m)-1 do
      begin
        if ((m[j].x=m[i].x-1) or (m[j].x=m[i].x+1)) and (m[j].y=m[i].y) then a:=true;
        if ((m[j].y=m[i].y-1) or (m[j].y=m[i].y+1)) and (m[j].x=m[i].x) then a:=true;
      end;
      if a then
      begin
        a:=false;
        for k:=0 to length(m)-1 do
          if k<>i then
          begin
            if ((m[k].x=m[j].x-1) or (m[k].x=m[j].x+1)) and (m[k].y=m[j].y) then a:=true;
            if ((m[k].y=m[j].y-1) or (m[k].y=m[j].y+1)) and (m[k].x=m[j].x) then a:=true;
          end;
      end;
      if a then
      begin
        SetLength(result, length(result)+1);
        result[length(result)-1]:=m[i];
      end;
    end;
    SetLength(m,length(result));
    SetLength(m1,length(0));
    SetLength(m2,length(0));
    m:=result;
    x:=0;
    y:=0;
    for i:=0 to length(m)-1 do
...
(*    SetLength(m, 0, 0);
    linecount:=0; colcount:=0; lines:=0;
    s2:='';
    for line := 4 to sl.Count - 1 do
    begin
      s1:= sl[line];
      j:=0; for i:=0 to length(s1)-1 do if s1[i]=ch then j:=j+1;
      SetLength(slRow,j+1);
      for i:=0 to length(slRow)-1 do slRow[i]:='';
      k:=j;
      j:=0;
   //   slrow[j]:=sl;
   //   i:=0;
      for i:=1 to length(s1) do
        if s1[i]<>ch then slRow[j] := slRow[j]+s1[i] else j:=j+1;
      for col := 0 to k do
      begin
        if col=0 then
          begin
            linecount:=linecount+1;
            if linecount>lines then lines:=linecount;
            SetLength(m,lines,colcount);
          end;
        if col=1 then
          begin
            if slRow[col]<>s2 then
            begin
              colcount:=colcount+1;
              linecount:=1;
            end;
            SetLength(m[linecount-1],colcount);
            s2:=slRow[col];
          end;
        if col=2 then m[linecount-1,colcount-1]:=slRow[col];  // Z = m[Y,X] !!!
      end;
    end; *)
  finally
    sl.Free;
...
//  for i:=0 to lines-1 do SetLength(m1[i],Length(m[i]));
  try
(*    for i:=0 to lines-1 do
      for j:=0 to length(m[i])-1 do
      begin
        if m[i,j]='' then m1[i,j]:=0 else
          begin
   //         for k:=1 to length(m[i,j]) do
   //           if m[i,j][k]='+' then m[i,j][k]:='-';
            m1[i,j]:=StrToFloat(m[i,j])/1E10;
          end;
        if m1[i,j]<0.2 then
        begin
  //        PaintBox2.Canvas.Pixels[PaintBox2.Width-i*amp,j*amp]:=clRed;
          l:=l+1;
        end;
      end;  *)
...
//    a:=false;
//    l:=(length(m2)+(3-(length(m2) mod 3))) div 3;   //numero de las fracciones
    SetLength(m_cola,l);
    o:=0;
    for i:=0 to k-1 do
    begin
...
//Triangulation:
    Triangulation(l,k, m_cola, m_con);
    i:=0;
    ...
    for i:=0 to length(m_point)-1 do
      PaintBox2.Canvas.Ellipse(m_point[i].x*amp-5,m_point[i].y*amp-5,m_point[i].x*amp+5,m_point[i].y*amp+5);
  finally
    Bitmap.Free;
    SetLength(m_cola,0);
    SetLength(m_con,0);
